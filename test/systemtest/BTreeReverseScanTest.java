...
public class BTreeReverseScanTest extends SimpleDbTestBase {
    ...
    @Test public void testRewindPredicates() throws IOException, DbException, TransactionAbortedException {
        ...
        ipred = new IndexPredicate(Op.GREATER_THAN, new IntField(r.nextInt(BTreeUtility.MAX_RAND_VALUE)));
        it = tuples.iterator();
        while(it.hasNext()) {
            ArrayList<Integer> tup = it.next();
            if(tup.get(keyField) >=((IntField) ipred.getField()).getValue()) {//>=已改
                tuplesFiltered.add(tup);
            }
        }
        ...
        // GREATER_THAN
        tuplesFiltered.clear();
        ipred = new IndexPredicate(Op.LESS_THAN_OR_EQ, new IntField(r.nextInt(BTreeUtility.MAX_RAND_VALUE)));
        it = tuples.iterator();
        while(it.hasNext()) {
            ArrayList<Integer> tup = it.next();
            if(tup.get(keyField) <((IntField) ipred.getField()).getValue()) {
    ...
    @Test public void testReadPage() throws Exception {
        ...
        // LESS_THAN
        tuplesFiltered.clear();
        ipred = new IndexPredicate(Op.GREATER_THAN, new IntField(r.nextInt(LEAF_PAGES*502)));
        it = tuples.iterator();
        while(it.hasNext()) {
            ArrayList<Integer> tup = it.next();
            if(tup.get(keyField) >=((IntField) ipred.getField()).getValue()) {
        ...
        // GREATER_THAN
        tuplesFiltered.clear();
        ipred = new IndexPredicate(Op.LESS_THAN_OR_EQ, new IntField(r.nextInt(LEAF_PAGES*502)));
        it = tuples.iterator();
        while(it.hasNext()) {
            ArrayList<Integer> tup = it.next();
            if(tup.get(keyField)< ((IntField) ipred.getField()).getValue()) {
